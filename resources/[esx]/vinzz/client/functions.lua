...
function ESX.ShowNotification(msg, type)
	...
	msg = string.gsub(msg,"~p~","")
	msg = string.gsub(msg,"~c~","")
	msg = string.gsub(msg,"~m~","")
	msg = string.gsub(msg,"~u~","")
	msg = string.gsub(msg,"~o~","")
	exports['ox_inventory']:notify({type = type, text = msg, duration = 3000})
end
...
function ESX.ShowHelpNotification(msg)
	if helpmessage ~= msg then
		...
		msg = string.gsub(msg,"~u~","")
		msg = string.gsub(msg,"~o~","")
		exports['vinsan_textui']:Show(msg)
	else
		lasthelp = GetGameTimer()
	end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if (GetGameTimer() - lasthelp) > 500 and helpmessage ~= '' then
			helpmessage = ''
			exports['vinsan_textui']:Hide()
		end
	end
end)
...
function ESX.Game.SpawnVehicle(vehicle, coords, heading, cb, networked, notgivekeys)
	...
	CreateThread(function()
		...
		if notgivekeys == nil then
			TriggerServerEvent('bixbi_vehkeys:AddKey', GetPlayerServerId(PlayerId()), ESX.Math.Trim(GetVehicleNumberPlateText(vehicle)))
		end
...
function ESX.Game.SpawnLocalVehicle(vehicle, coords, heading, cb)
	ESX.Game.SpawnVehicle(vehicle, coords, heading, cb, false, true)
end
...
function ESX.Game.GetVehicleProperties(vehicle)
	if DoesEntityExist(vehicle) then
		local nitroFuel, nitroStorage
		local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
		local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle)
		local numWheels  = tostring(GetVehicleNumberOfWheels(vehicle))
        local smashWindow, doorsBroken, tyreBurst, extras = {}, {}, {}, {}
        local TyresIndex = {
            ['2'] = {0, 4},
            ['3'] = {0, 4, 5},
            ['4'] = {0, 1, 4, 5},
        ...
        for tyre,idx in pairs(TyresIndex[numWheels]) do
            local idxString = tostring(idx)
            if IsVehicleTyreBurst(vehicle, idx, false) then
                tyreBurst[idxString] = true
            else
                tyreBurst[idxString] = false
            end 
        end
        
        for eWindowId = 0, 7 do
            if IsVehicleWindowIntact(vehicle, eWindowId) then 
                smashWindow[tostring(eWindowId)] = false
            else
                smashWindow[tostring(eWindowId)] = true
            end
        end
		...
		if IsToggleModOn(vehicle, 18) then
			if DecorExistOn(vehicle, 'nitroFuel') then
				nitroFuel = DecorGetInt(vehicle, 'nitroFuel')
			else
				nitroFuel = false
			end

			if DecorExistOn(vehicle, 'nitroStorage') then
				nitroStorage = DecorGetInt(vehicle, 'nitroStorage')
			else
				nitroStorage = false
			end
		end
...
function ESX.Game.SetVehicleProperties(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.tankHealth then SetVehiclePetrolTankHealth(vehicle, 1000.0) end
		...
		if props.nitroFuel and props.nitroFuel ~= false then
			DecorSetInt(vehicle, 'nitroFuel', props.nitroFuel)
		end

		if props.nitroStorage and props.nitroStorage ~= false then
			DecorSetInt(vehicle, 'nitroStorage', props.nitroStorage)
		end
		...
		if props.smashWindow then
            for k, v in pairs(props.smashWindow) do
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to', {
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									...
									ESX.ShowNotification(_U('in_vehicle'), 'error')
								end
							else
								ESX.ShowNotification(_U('players_nearby'), 'error')
								menu2.close()
							end
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity and quantity > 0 then
									if pedAmmo >= quantity then
										...
										ESX.ShowNotification(_U('noammo'), 'error')
									end
								else
									ESX.ShowNotification(_U('amount_invalid'), 'error')
								end
