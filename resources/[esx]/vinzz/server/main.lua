SetMapName('Spaceman Roleplay Indonesia')
SetGameType('#SMRP')
	
local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `group` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `loadout`, `reward`, `currentped`'
...
if Config.Multichar then
	AddEventHandler('esx:onPlayerJoined', function(src, char, data)
		...
		if not ESX.Players[src] then
			...
			ESX.SendDiscord(GetPlayerName(src), 'Telah memasuki server ['..ESX.GetIdentifier(src)..']', 'playerjoinleft')
			if data then
...
RegisterNetEvent('esx:savePed')
AddEventHandler('esx:savePed', function(pedname)
	local xPlayer = ESX.GetPlayerFromId(source)

	exports.oxmysql:execute('UPDATE users SET currentped = @currentped WHERE identifier = @identifier', {
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	if result.position and result.position ~= '' then
		...
		userData.coords = {x = -516.9364, y = -252.6937, z = 34.6557, heading = 208.8475}
	end
	...
	if result.reward and result.reward ~= '' then
		userData.reward = result.reward
	end

	if result.currentped and result.currentped ~= '' then
		userData.currentped = result.currentped
	else
		userData.currentped = false
	end
	...
	if userData.firstname then 
		...
		if userData.reward then xPlayer.set('reward', userData.reward) end
		if userData.currentped then xPlayer.set('currentped', userData.currentped) end
	end
	...
	if isNew then
		for k,v in pairs(Config.StarterItem) do
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		...
		ESX.SendDiscord(GetPlayerName(playerId), 'Telah keluar dari server ['..ESX.GetIdentifier(playerId)..'] Dengan Alasan : ['..reason.. ']', 'playerjoinleft')
	end
end)
