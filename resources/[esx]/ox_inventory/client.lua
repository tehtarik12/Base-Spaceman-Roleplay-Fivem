...
local function canOpenTarget(ped)
    local srvId = GetPlayerServerId(NetworkGetPlayerIndexFromPed(ped))
    return IsPedFatallyInjured(ped)
    or IsEntityPlayingAnim(ped, 'dead', 'dead_a', 3)
    or IsEntityPlayingAnim(ped, 'mini@ @char_b@cpr_def', 'cpr_pumpchest_idle', 3)
    ...
    or IsEntityPlayingAnim(ped, 'random@mugging3', 'handsup_standing_base', 3)
    or exports.wasabi_ambulance:isPlayerDead(srvId)
end
...
RegisterNetEvent('ox_inventory:giveAnim', function()
...
RegisterNUICallback('giveItem', function(data, cb)
    ...
    for i = 1, #players, 1 do
        if players[i] ~= PlayerId() then
            ...
            table.insert(GiveMenu,  {
                header = GetPlayerServerId(players[i]),
                server = true,
                event = "ox_inventory:giveItem",
                args = {
                    slot = data.slot,
    ...
    if not foundPlayers then
        ESX.ShowNotification('No Citizens Nearby!')
    else
		TriggerEvent('nh-context:createMenu', GiveMenu)
		-- Utils.PlayAnim(2000, 'mp_common', 'givetake1_a', 1.0, 1.0, -1, 50, 0.0, 0, 0, 0)
		if data.slot == currentWeapon?.slot then
			currentWeapon = Weapon.Disarm(currentWeapon)
		end
