...
local function dropItem(source, data)
	...
	local fortisLogsTable = {
		["nama"] = playerInventory.label,
		["jumlah"] = data.count,
		["todata"] = toData.name,
	}
	exports["fortislogs"]:addLog("Drop Item", fortisLogsTable) 
					...
					if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
						...
						if not sameInventory then
							if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
								...
								local fortisLogsTable = {
									["namaitem"] = fromData.name,
									["jumlah"] = data.count,
									["dari"] = fromInventory.label,
									["ke"] = toInventory.label,
								}
								exports["fortislogs"]:addLog("Perpindahan Barang", fortisLogsTable) 
...
RegisterServerEvent('ox_inventory:giveItem', function(args)
	local slot, target, count = args.slot, args.target, args.count
	...
	if toInventory?.player then
		...
		if not toInventory.open and data and data.count >= count and Inventory.CanCarryItem(toInventory, item, count, data.metadata) and TriggerEventHooks('swapItems', {
			...
			TriggerClientEvent('ox_inventory:giveAnim', source)
			
			local fortisLogsTable = {
				["source"] = fromInventory.label,
				["target"] = toInventory.label,
				["namaitem"] = data.name,
				["jumlah"] = count,
			}
			exports["fortislogs"]:addLog("Give Item ke inventory", fortisLogsTable) 
			if server.loglevel > 0 then
...
RegisterNetEvent('ox:privatestash', function(data)
	RegisterStash(data.stash.name, data.stash.label, data.stash.slots, data.stash.weight, true)
end)
