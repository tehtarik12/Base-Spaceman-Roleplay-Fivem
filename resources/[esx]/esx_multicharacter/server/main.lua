...
elseif ESX.GetConfig().Multichar == true then
	...
	local function SetupCharacters(source)
		...
		local skinresult = MySQL.query.await('SELECT skin FROM users WHERE identifier LIKE ?', {identifier})
		local characters
		...
		if result then
			...
			for i = 1, characterCount, 1 do
				local v = result[i]
				local reskin = nil
				if skinresult[i] and skinresult[i].skin then reskin = skinresult[i].skin end
				...
				characters[id] = {
					...
					skin = reskin and json.decode(reskin) or {},
					disabled = v.disabled,
	...
	MySQL.ready(function()
		...
		if DB_COLUMNS then
			...
			local function GetJobs()
				...
				return exports.vinzz:getSharedObject().Jobs
			end
			...
			FETCH = 'SELECT identifier, accounts, job, job_grade, firstname, lastname, dateofbirth, sex, disabled FROM users WHERE identifier LIKE ? LIMIT ?'
		end
	end)
	...
	RegisterNetEvent('esx_multicharacter:CharacterChosen', function(charid, isNew)
		if type(charid) == 'number' and string.len(charid) <= 2 and type(isNew) == 'boolean' then
			if isNew then
				...
				ESX.Players[identifier] = true
				local skinresult = MySQL.query.await('SELECT skin FROM users WHERE identifier LIKE ?', {PREFIX..'%:'..identifier})
				if skinresult[1] and skinresult[1].skin then
					skinresult = true
				else
					skinresult = false
				end
				local haveSpawned = true
				local properties, jail = {}, 0

				if spawnedPlayer[identifier] == nil or os.time() - spawnedPlayer[identifier] > 3600 then haveSpawned = false end
				TriggerClientEvent('vinsan_core:SpawnSelector', src, haveSpawned, properties, jail, skinresult)
			end
		end
	end)
	...
	AddEventHandler('playerDropped', function(reason)
		...
		spawnedPlayer[GetIdentifier(source)] = os.time()
	end)
