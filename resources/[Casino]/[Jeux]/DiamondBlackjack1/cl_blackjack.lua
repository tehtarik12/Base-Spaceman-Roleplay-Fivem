...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(-307.1258, 227.1163, 77.9405),
        dealerHeading = 183.7768,
        tablePos = vector3(-306.971, 226.2574, 76.93926),
        tableHeading = 8.0729608535767,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [1] = {
        dealerPos = vector3(-307.4625, 229.7999, 77.9405),
        dealerHeading = 1.2433,
        tablePos = vector3(-307.5953, 230.6588, 76.93926),
        tableHeading = 188.07302856445,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [2] = {
        dealerPos = vector3(-302.3432, 230.6785, 77.9405),
        dealerHeading = 4.3127,
        tablePos = vector3(-302.4922, 231.48, 76.93926),
        tableHeading = 188.07302856445,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [3] = {
        dealerPos = vector3(-301.8417, 227.8061, 77.9405),
        dealerHeading = 187.6919,
        tablePos = vector3(-301.6973, 227.0054, 76.93926),
        tableHeading = 8.0729608535767,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Tekan E Untuk Bermain Blackjack")
                    else 
                        drawNativeNotification("Kursi Telah Di Isi")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Jumlah Kartu di tangan Anda ["..tostring(currentHand).."] - Jumlah Kartu Dealer ["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("Jumlah Taruhan")
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "Taruhan:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            ...
            DrawAdvancedNativeText(1.013, 0.892, 0.005, 0.0028, 0.29, "Waktu:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Pasang Taruhan Anda")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Tidak ada taruhan yang dipasang, putaran dilewati")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Silahkan menunggu permainan berikutnya")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("Anda Kurang Beruntung, silahkan coba kembali")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("Seri, Tidak ada yang menang")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("Selamat Anda Menang! ")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
