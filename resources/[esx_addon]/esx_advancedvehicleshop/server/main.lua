...
--[[RegisterCommand('masukin', function(source, args, rawcommand)
...
ESX.RegisterServerCallback('esx_advancedvehicleshop:buyVehicle', function(source, cb, pChoiceS, pJobS, sTypeS, vModelS, vPlateS, vCatS, vNameS, vImageS)
	...
	elseif pChoiceS == 'payfull' then
		if modelPrice and xPlayer.getMoney() >= modelPrice then
			...
			if vModelS == Config.Division.Heli1 or vModelS == Config.Division.Heli2 then
				type = 'aircraft'
				category = 'helis'
			else
...
RegisterCommand('givecar', function(source, args, rawCommand)
	...
	if xPlayer and xPlayer.getGroup() == 'superadmin' then
		if not xTarget then
			xPlayer.showNotification('Target tidak ditemukan')
			return
		end

		if not args[1] or not args[2] then
			xPlayer.showNotification('/givecar playerID carModel [plate]')
		elseif args[3] then
			local plate = args[3]
			if #args > 3 then
				for i=4, #args do
			...
			TriggerClientEvent('esx_giveownedcar:spawnVehicle', xTarget.source, args[2], plate)
			ESX.SendDiscord(GetPlayerName(source), 'Telah Memberikan Kendaraan **'.. args[2] ..'** Kepada **'.. GetPlayerName(args[1]) .. '** Dengan Plate **' .. args[3] ..'**', 'commandrunned')

		else
			TriggerClientEvent('esx_giveownedcar:spawnVehicle', xTarget.source, args[2], nil)
			ESX.SendDiscord(GetPlayerName(source), 'Telah Memberikan Kendaraan **'.. args[2] ..'** Kepada **'.. GetPlayerName(args[1]) ..'**', 'commandrunned')

		end
	elseif xPlayer.getGroup() ~= 'superadmin' then
		xPlayer.showNotification('Anda tidak dapat melakukan tindakan tersebut!')
	end
end, true)

RegisterServerEvent('esx_giveownedcar:setVehicle')
AddEventHandler('esx_giveownedcar:setVehicle', function (vehicleProps, vehiclename)
	local xPlayer = ESX.GetPlayerFromId(source)
	local category, name = 'Unknown', 'Unknown'

	for k,v in ipairs(vehiclesvc) do
		if vehiclename == v.model then
			category = v.category
			name = v.name
			break
		end
	...
	if xPlayer then
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, type, job, stored, last_garage, garage, category, name, image) VALUES (@owner, @plate, @vehicle, @type, @job, @stored, @last_garage, @garage, @category, @name, @image)', {
			['@owner'] = xPlayer.identifier,
			['@plate'] = vehicleProps.plate,
			['@job'] = 'civ',
			['@vehicle'] = json.encode({model = GetHashKey(vehiclename), plate = vehicleProps.plate}),
			['@type'] = 'car',
			['@category'] = category,
			['@name'] = name,
			['@last_garage'] = 'walkot',
...
RegisterServerEvent('esx_giveownedcar:setVehicleReward')
AddEventHandler('esx_giveownedcar:setVehicleReward', function (vehicleProps, vehiclename)
	local xPlayer = ESX.GetPlayerFromId(source)
	local category, name = suvs, toros
