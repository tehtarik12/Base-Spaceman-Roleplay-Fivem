...
RegisterCommand('fixdealer', function()
	if Config.Ambulance.Blips or Config.Police.Blips or Config.Division.Blips or Config.Mechanic.Blips or Config.Taxi.Blips then
		RefreshJobBlips()
	end
...
function BuyVehicleShopMenu(pJobC, sTypeC)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_buy', {
		...
		align = GetConvar('esx_MenuAlign', 'top-right'),
		elements = elements
	}, function(data, menu)
		local elements2 = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_confirm', {
			...
			if purchaseChoice == 'finance' and not YesAlready or purchaseChoice == 'payfull' and not YesAlready then
				...
				if pJobC == 'ambulance' and sTypeC == 'car' or pJobC == 'police' and sTypeC == 'car' or pJobC == 'mechanic' and sTypeC == 'car' or pJobC == 'taxi' and sTypeC == 'car' then
					if ESX.PlayerData.job and ESX.PlayerData.job.name == pJobC and ESX.PlayerData.job.grade >= tonumber(vehicleData.category) then
				...
				elseif pJobC == 'ambulance' and sTypeC == 'division' or pJobC == 'police' and sTypeC == 'division' then
					if vehicleData.model == Config.Division.Heli1 or vehicleData.model == Config.Division.Heli2 then
						ContinuePurchase = true
						spawnloc, spawnhead = thisShop.Outside2, thisShop.OutsideH2
					else
						ContinuePurchase = true
					end
				elseif (sTypeC == 'vipcar' and ESX.PlayerData.job.name == 'dealer') or sTypeC ~= 'vipcar' then
					ContinuePurchase = true
				end
...
function CreateBlips()
	if Config.Aircraft.Shop and Config.Aircraft.Blips then
		for k,v in pairs(Config.Aircraft.Locations) do
			...
			AddTextComponentSubstringPlayerName('[VEH] '.._U('aircraft_dealer'))
	...
	if Config.Boat.Shop and Config.Boat.Blips then
		for k,v in pairs(Config.Boat.Locations) do
			...
			BeginTextCommandSetBlipName('STRING')
			AddTextComponentSubstringPlayerName('[VEH] '.._U('boat_dealer'))
			EndTextCommandSetBlipName(blip)
			table.insert(RegBlips, blip)
		end
	end
	...
	if Config.Truck.Shop and Config.Truck.Blips then
		for k,v in pairs(Config.Truck.Locations) do
			...
			BeginTextCommandSetBlipName('STRING')
			AddTextComponentSubstringPlayerName('[VEH] '.._U('truck_dealer'))
			EndTextCommandSetBlipName(blip)
			table.insert(RegBlips, blip)
		end
	end
	...
	if Config.VIPAircraft.Shop and Config.VIPAircraft.Blips then
		for k,v in pairs(Config.VIPAircraft.Locations) do
			...
			BeginTextCommandSetBlipName('STRING')
			AddTextComponentSubstringPlayerName('[VEH] '.._U('vip_dealer'))
			EndTextCommandSetBlipName(blip)
			table.insert(RegBlips, blip)
		end
	end
...
AddEventHandler('esx_giveownedcar:spawnVehicle', function(model, plate)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local generatedPlate = not plate and GeneratePlate() or string.upper(plate)
		...
		if not isPlateTaken then
			ESX.Game.SpawnVehicle(model, coords, 0.0, function(vehicle) --get vehicle info
				if DoesEntityExist(vehicle) then
					...
					if model == 'toros' then
						TriggerServerEvent('esx_giveownedcar:setVehicleReward', vehicleProps, model)
						ESX.ShowNotification(_U('gived_carReward', model, generatedPlate))
					else
						TriggerServerEvent('esx_giveownedcar:setVehicle', vehicleProps, model)
						ESX.ShowNotification(_U('gived_car', model, generatedPlate))
					end
