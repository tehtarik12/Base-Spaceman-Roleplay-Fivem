	...
	for i=1, #Status, 1 do
		for j=1, #status, 1 do
			if Status[i].name == status[j].name then
				...
				if Status[i].name == 'armour' and status[j].val >= 1 then
					SetPedArmour(PlayerPedId(), status[j].val)
				elseif Status[i].name == 'health' and status[j].val >= 1 then
					SetEntityHealth(PlayerPedId(), status[j].val)
				end
			end
		end
	end
