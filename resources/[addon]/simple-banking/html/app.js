...
function addcomment(comment)
{
    return "<div>\
        <hr>\
            <h6 class='comment'>Note</h6>\
            <p class='comment'>" + htmlEncode(comment) + "</p>\
        </hr>\
    </div>"
}
function AddTransaction(trans_id, account, amount, time, comment, why, receiver, plName)
{
            ...
            <div class='card-body'>\
                ...
                <div class='row game-header'>\
                    ...
                    <div class='col-5'>\
                        ...
                        <div class='col d-flex justify-content-end'>\
                            <div>\
                                <p class='comment' id='"+trans_id+"_time'>" + TimeSince + " ago</p>\
                                <p class='comment'> " + plName + "</p>\
                            </div>\
                        </div>\
...
function OpenATM(data, transactions, name)
{
    ...
    if (transactions && transactions !== null)
    {
        ...
        for (var i = 0; i < transactions.length; i++)
        {
            ...
            AddTransaction(tTbl.trans_id, tTbl.account, tTbl.amount, tTbl.date, tTbl.comment, tTbl.trans_type, tTbl.receiver || "Unknown", name);
        }
    }
...
function confirmDeposit()
{
    ...
    let comment = $("#depositcomment").val();
...
function confirmWithdraw()
{
    ...
    let comment = $("#withdrawcomment").val();
    $(".modal").hide();
...
function confirmTransfer()
{
    ...
    let comment = $("#transfercomment").val();
...
Listeners["update_transactions"] = function(data)
{
    ...
    {
        let tTbl = transactions[i];
        AddTransaction(tTbl.trans_id, tTbl.account, tTbl.amount, tTbl.date, tTbl.comment, tTbl.trans_type, tTbl.receiver || "Unknown", playerName);
    }
}
