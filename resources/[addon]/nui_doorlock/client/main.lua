if not ESX then
	...
	RegisterNetEvent('vinsan_core:lockpick') -- Set up your own lockpick event here
	AddEventHandler('vinsan_core:lockpick', function()
		if not IsEntityPlayingAnim(PlayerPedId(), 'dead', 'dead_a', 3) and not isCuffed and (closestDoor and closestDoor.data and closestDoor.data.lockpick) then
			local latest = closestDoor.data
			TaskTurnPedToFaceCoord(PlayerPedId(), latest.textCoords, 0)

			exports.rprogress:MiniGame({
				Zone = 40,
				Duration = 1250,
				Timeout = 1250, -- Duration before minigame is cancelled
				onComplete = function(success)
					if success then
						local locked = not latest.locked
