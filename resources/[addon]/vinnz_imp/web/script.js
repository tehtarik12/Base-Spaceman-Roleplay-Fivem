$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://vinnz_imp/escape', JSON.stringify({}));
		}
	};
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://vinnz_imp/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	function validateImpoundForm () {
		...
		if(reason.length < rules.minReasonLength || reason.length > 350000) {
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://vinnz_imp/unimpound', JSON.stringify(plate));
	});
	...
	$('#cancel, #exit').click(function (event) {
		$.post('http://vinnz_imp/escape', null);
	});
