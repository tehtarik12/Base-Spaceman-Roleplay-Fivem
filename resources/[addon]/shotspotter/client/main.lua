...
function createBlip(data)
	Citizen.CreateThread(function()
		...
		local entCoord = GetEntityCoords(data.netId)
		if data.netId and entId > 0 then
			blip = AddBlipForRadius(data.coords.x, data.coords.y, data.coords.z, 100.0)
			...
			RemoveBlip(blip)
			Citizen.Wait(0)
			blip = AddBlipForRadius(GetEntityCoords(data.netId))
		else
			data.netId = nil
			blip = AddBlipForRadius(data.coords.x, data.coords.y, data.coords.z, 100.0)
		end
...
RegisterKeyMapping('alert_gps', '[UT] Set waypoint', 'keyboard', 'Y')
				...
				if GetVehiclePedIsUsing(playerPed) ~= 0 then
							...
							if (GetEntitySpeed(vehicle) * 30.6) >= (speedlimit + (math.random(30,60))) then
								if zoneChance('Speeding', 4, currentStreetName) then
									...
									if IsPedInAnyVehicle(playerPed, true) and ((GetEntitySpeed(vehicle) * 3.6) >= (speedlimit + (math.random(30,60)))) then
										local veh = vehicleData(vehicle)
										data = {dispatchCode = 'speeding', caller = _U('caller_local'), coords = playerCoords, netId = veh.id, priority = 'low', info = ('[%s] %s%s'):format(veh.plate, veh.doors, veh.class), info2 = veh.colour, units = {length = 0}}
										TriggerServerEvent('wf-alerts:svNotify', data)
					...
					if Config.Timer['Shooting'] == 0 and not BlacklistedWeapon(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and IsPedArmed(playerPed, 4) then
						...
						if IsPedShooting(playerPed) and zoneChance('Shooting', 2, currentStreetName) then
							data = {dispatchCode = 'shooting', caller = _U('caller_local'), coords = playerCoords, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 6000, units = {length = 0}}
							TriggerServerEvent('wf-alerts:svNotify', data)
							Config.Timer['Shooting'] = Config.Shooting.Success
					...
					elseif Config.Timer['Melee'] == 0 and IsPedInMeleeCombat(playerPed) and HasPedBeenDamagedByWeapon(GetMeleeTargetForPed(playerPed), 0, 1) then
						...
						if zoneChance('Melee', 3, currentStreetName) then
							data = {dispatchCode = 'melee', caller = _U('caller_local'), coords = playerCoords, netId = NetworkGetNetworkIdFromEntity(playerPed), length = 4000, units = {length = 0}}
							TriggerServerEvent('wf-alerts:svNotify', data)
...
RegisterKeyMapping('alert_dead', '[UT] Send distress signal to Police/EMS', 'keyboard', 'G')
...
RegisterCommand('911', function(playerId, args, rawCommand)
	...
	if Config.Default911 then TriggerServerEvent('mdt:newCall', args, caller, playerCoords, data) else
