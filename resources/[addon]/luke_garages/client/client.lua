...
local function ImpoundBlips(coords, type, label, blipOptions)
    ...
    SetBlipSprite(blip, blipOptions?.sprite and blipOptions.sprite or type == 'car' and Config.Blip.Car or type == 'boat' and Config.Blip.Boat or Config.Blip.Aircraft)
    SetBlipScale(blip, 0.8)
    SetBlipColour(blip, 47)
    SetBlipAsShortRange(blip, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(label or '[VEH] ' .. Locale(type) .. ' ' .. Locale('impound_lot'))
    EndTextCommandSetBlipName(blip)
end

local function GarageBlips(coords, type, label, job, blipOptions, hideblip)
    if job then return end
    if hideblip then return end
    local blip = AddBlipForCoord(coords)
    SetBlipSprite(blip, blipOptions?.sprite and blipOptions.sprite or type == 'car' and Config.Blip.Car or type == 'boat' and Config.Blip.Boat or Config.Blip.Aircraft)
    SetBlipScale(blip, 0.8)
...
local function JobGarageBlip(garage)
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.SplitGarages == true and garage.label or '[VEH] ' .. Locale(garage.type) .. ' ' .. Locale('garage'))
...
for k, v in pairs(Config.Garages) do
    ...
    GarageBlips(vector3(v.pedCoords.x, v.pedCoords.y, v.pedCoords.z), v.type, v.label, v.job, v.blip, v.hideblip)
