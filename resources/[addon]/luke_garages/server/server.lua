...
if Config.SetImpoundVehicle then
    MySQL.ready(function()
        MySQL.Async.execute("UPDATE `owned_vehicles` SET `stored` = 0, `garage` = `last_garage` WHERE `stored` = 0")
    end)
end
...
lib.callback.register('luke_garages:GetImpound', function(source, type)
    ...
    local results = MySQL.Sync.fetchAll('SELECT `plate`, `vehicle`, `job` FROM owned_vehicles WHERE (`owner` = @identifier OR `owner` = @job) AND `type` = @type AND `stored` = 0 AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles)' , {
        ['@identifier'] = identifier,
        ['@type'] = type,
    ...
    MySQL.Async.fetchAll('SELECT vehicle, plate, garage FROM `owned_vehicles` WHERE plate = @plate', {['@plate'] = ESX.Math.Trim(plate)}, function(result)
        if result[1] then
            CreateThread(function()
                TriggerEvent('vinsan_vehkeys:ClearKey', ESX.Math.Trim(plate))
				...
				TriggerEvent('vinz_stancer:entitycreate', entity)
                TriggerEvent('vinsan_vehkeys:AddKey', xPlayer.source, ESX.Math.Trim(plate))
            end)
        end
    end)
end)
