...
local zoomspeed = 6.5 -- camera zoom speed
local speed_lr = 6.5 -- speed by which the camera pans left-right 
local speed_ud = 6.5 -- speed by which the camera pans up-down
...
local lightup_key = 172 -- control id to increase spotlight brightness. Default: INPUT_MP_TEXT_CHAT_TEAM (ARROW-UP)
...
local maxtargetdistance = 500.0 -- max distance at which target lock is maintained
...
Citizen.CreateThread(function() -- Register ped decorators used to pass some variables from heli pilot to other players (variable settings: 1=false, 2=true)
	while true do
		...
		if NetworkIsSessionStarted() then
			...
			DecorRegister("Cooldown", 3) -- Backup method of target ID
			break
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if not helicam then
				if IsHeliHighEnough(heli) and IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
					PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
					helicam = true
				end

				if IsControlJustPressed(0, toggle_spotlight) and GetPedInVehicleSeat(heli, -1) == lPed then -- Toggle forward and tracking spotlight states
			...
			elseif helicam then
				...
				while helicam and not IsEntityDead(lPed) and IsHeliHighEnough(heli) do
					...
					if locked_on_vehicle then
						if DoesEntityExist(locked_on_vehicle) then
							...
							local target_distance = #(vector3(coords1.x, coords1.y, coords1.z) - vector3(coords2.x, coords2.y, coords2.z))
