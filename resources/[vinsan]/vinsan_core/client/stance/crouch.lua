...
NormalWalk = function() 
	...
	TriggerEvent('kcmcity_stance:DefaultWalkStyle')
	Crouched = false
end
...
CanCrouch = function()
	...
	if IsPedOnFoot(PlayerPed) and not IsPedJumping(PlayerPed) and not IsPedFalling(PlayerPed) and not IsEntityPlayingAnim(PlayerPed, 'dead', 'dead_a', 3) then
		return true
	else
		return false
	end
...
RegisterKeyMapping('crouch', '[SC] Crouch', 'keyboard', 'LCONTROL') -- now its better player can change to any bottom they want
