...
local function playerConnect(name, setKickReason, deferrals)
    ...
    update(string_format(ConfigQueue.Language.pos .. ((Queue:TempSize() and ConfigQueue.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"), pos, Queue:GetSize(), ""))
    ...
    while true do
        ...
        local msg = string_format(ConfigQueue.Language.pos .. ((Queue:TempSize() and ConfigQueue.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or ""), pos, Queue:GetSize(), qTime)
        update(msg, data.deferrals)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= ConfigQueue.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
...
ESX.RegisterServerCallback('ConnectQueue:getSize', function(source, cb)
    cb(Queue:GetPlayerCount(), Queue:GetSize())
end)
