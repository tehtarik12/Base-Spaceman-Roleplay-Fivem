...
RegisterCommand("race", function(source, args)
    ...
    elseif args[1] == "start" then
        ...
        if amount then
            ...
            startDelay = startDelay and startDelay*1000 or Config.RacesConfig.joinDuration
...
AddEventHandler("StreetRaces:loadRace_cl", function(checkpoints)
    ...
    for index, checkpoint in pairs(recordedCheckpoints) do
        ...
        SetBlipColour(checkpoint.blip, Config.RacesConfig.checkpointBlipColor)
    ...
    SetBlipRouteColour(checkpoints[1].blip, Config.RacesConfig.checkpointBlipColor)
end)
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if Config.RacesConfig.checkpointRadius > 0 then
                        local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                        checkpoint.checkpoint = CreateCheckpoint(checkpointType, checkpoint.coords.x,  checkpoint.coords.y, checkpoint.coords.z, 0, 0, 0, Config.RacesConfig.checkpointRadius, 255, 255, 0, 127, 0)
                        SetCheckpointCylinderHeight(checkpoint.checkpoint, Config.RacesConfig.checkpointHeight, Config.RacesConfig.checkpointHeight, Config.RacesConfig.checkpointRadius)
                    end

                    -- Set blip route for navigation
                    SetBlipRoute(checkpoint.blip, true)
                    SetBlipRouteColour(checkpoint.blip, Config.RacesConfig.checkpointBlipColor)
                else
                    -- Check player distance from current checkpoint
                    local checkpoint = race.checkpoints[raceStatus.checkpoint]
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < Config.RacesConfig.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if Config.RacesConfig.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 0, 0, 0, Config.RacesConfig.checkpointRadius, 255, 255, 0, 127, 0)
                                SetCheckpointCylinderHeight(nextCheckpoint.checkpoint, Config.RacesConfig.checkpointHeight, Config.RacesConfig.checkpointHeight, Config.RacesConfig.checkpointRadius)
                            end

                            -- Set blip route for navigation
                            SetBlipRoute(nextCheckpoint.blip, true)
                            SetBlipRouteColour(nextCheckpoint.blip, Config.RacesConfig.checkpointBlipColor)
                        end
                    end
                end

                -- Draw HUD when it's enabled
                if Config.RacesConfig.hudEnabled then
                    ...
                    Draw2DText(Config.RacesConfig.hudPosition.x, Config.RacesConfig.hudPosition.y, ("~y~%02d:%06.3f"):format(timeMinutes, timeSeconds), 0.7)
                    local checkpoint = race.checkpoints[raceStatus.checkpoint]
                    ...
                    Draw2DText(Config.RacesConfig.hudPosition.x, Config.RacesConfig.hudPosition.y + 0.04, ("~y~CHECKPOINT %d/%d (%dm)"):format(raceStatus.checkpoint, #race.checkpoints, checkpointDist), 0.5)
                end
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= Config.RacesConfig.freezeDuration then
                    -- Display countdown text and freeze vehicle position
                    Draw2DText(0.5, 1.0, ("~y~%d"):format(math.ceil(count/1000.0)), 3.0)
            ...
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < Config.RacesConfig.joinProximity and currentTime < race.startTime then
                        -- Draw 3D text
                        ...
                        if IsControlJustReleased(1, Config.RacesConfig.joinKeybind) then
        ...
        if raceStatus.state == RACE_STATE_RECORDING then
            ...
            if IsWaypointActive() then
                ...
                if (coords ~= nil) then
                    ...
                    SetBlipColour(blip, Config.RacesConfig.checkpointBlipColor)
